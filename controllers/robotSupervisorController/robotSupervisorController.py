...
class CartpoleRobot(RobotSupervisorEnv):
    def __init__(self):
        ...
        self.positionSensor = self.getDevice("inertial unit")
        self.positionSensor.enable(self.timestep)

        self.poleEndpoint = self.getFromDef("Solid_top")
        if(self.poleEndpoint==None): exit(1)
        
        self.wheels = []
        for wheelName in ['motor_right', 'motor_left']:
    ...
    def get_observations(self):
        ...
        poleAngle = normalizeToRange(self.positionSensor.getRollPitchYaw()[1], -0.23, 0.23, -1.0, 1.0, clip=True)
    ...
    def is_done(self):
        ...
        poleAngle = round(self.positionSensor.getRollPitchYaw()[1], 2)
